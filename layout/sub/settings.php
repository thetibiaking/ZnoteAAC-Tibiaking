...
if (isset($_GET['success']) === true && empty($_GET['success']) === true) {
	...
	<form action="" method="post">
        ...
        <br>
        <input class="form-control" type="text" name="new_email" value="<?php echo $user_data['email']; ?>">
        <br>
        <label for="new_flag">Country:</label>
        <br>
        <select class="form-control" data-size="5" name="new_flag" id="flag_select">
        <option value="">(Please choose)</option>
        <?php
        foreach(array('pl', 'se', 'br', 'us', 'gb', ) as $c)
            echo '<option class="form-control" value="' . $c . '">' . $config['countries'][$c] . '</option>';
