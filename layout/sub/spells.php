...
if (user_logged_in() && is_admin($user_data)) {
	if (isset($_GET['update'])) {
		...
		?>
		<form action="">
            <button type="submit" name="update" class="btn btn-danger">Generate new cache</button>
		</form>
		<?php
	}
...
<article class="blog_item">
	...
	<form action="#spells" class="filter_spells form-inline">
        <label class="my-1 mr-2" for="vocation">Filter vocation:</label>
        <select class="custom-select my-1 mr-sm-2" id="vocation" name="vocation">
            <option class="form-control" value="all">All</option>
            <?php foreach ($config['vocations'] as $id => $vocation): ?>
                <option class="form-control" value="<?php echo $id; ?>" <?php if ($getVoc === $id) echo "selected"; ?>><?php echo $vocation['name']; ?></option>
            <?php endforeach; ?>
        </select>
    ...
    <a href="#spells" class="btn btn-primary" role="button">Back to top <i class="fas fa-caret-up"></i></a>
    <br>
    <br>
    <table class="table">
