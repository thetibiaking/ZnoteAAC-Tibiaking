	...
	<tr class="yellow">
		...
		if ($inv_data !== false) foreach ($inv_data as $inv) {
			...
			?> <form action="" method="post"> <?php
				echo '<td>';
				echo '<input class="form-control" type="hidden" name="uninvite" value="' . $inv['player_id'] . '" />';
				echo '<input class="form-control" type="submit" value="Remove Invitation">';
				echo '</td>';
			?> </form> <?php
			}
			// Join Guild
			?> <form action="" method="post"> <?php
				for ($i = 0; $i < $char_count; $i++) {
					if ($charactersId[$i] == $inv['player_id']) {
						echo '<td>';
						echo '<input class="form-control" type="hidden" name="joinguild" value="' . $inv['player_id'] . '" />';
						echo '<input class="form-control" type="submit" value="Join Guild">';
						echo '</td>';
						$bool = true;
					}
				}
